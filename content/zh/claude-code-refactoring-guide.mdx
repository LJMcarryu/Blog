...
重构这个话题，在这个系列里其实一直是"配角"——[进阶指南](/zh/blog/claude-code-advanced-guide)的 9.3 节用了 5 行写了个流程清单，[测试指南](/zh/blog/claude-code-testing-guide)里把重构当作 TDD 循环的最后一步，[Git 指南](/zh/blog/claude-code-git-guide)里提到了重构分支策略。但从来没有一篇文章正面回答过：**怎么用 Claude Code 系统性地做重构**。

提取函数怎么提？大文件怎么拆？类型系统怎么迁移？API 怎么做 breaking change？跨几十个文件的重命名怎么保证不漏？这些问题，今天一次讲清楚。

这篇文章和[调试指南](/zh/blog/claude-code-debugging-guide)是姊妹篇——调试是"代码出了问题怎么修"，重构是"代码没问题但可以更好"。两篇搭配着看效果更好。
...
没有测试的重构就是在走钢丝。详细的测试工作流参考[测试指南](/zh/blog/claude-code-testing-guide)：
...
重构一定要在独立分支上做。推荐用 worktree 隔离，详见 [Git 指南](/zh/blog/claude-code-git-guide)：
...
关键提示词技巧（更多参考 [Prompt 指南](/zh/blog/claude-code-prompt-guide)）：
...
对于互不依赖的模块，用多个 Claude Code 实例并行。详见[多 Agent 指南](/zh/blog/claude-code-multi-agent-guide)：
...
风险较高的重构，用 Git worktree 隔离最安全（详见 [Git 指南](/zh/blog/claude-code-git-guide)）。主工作区不受影响，失败直接删除 worktree 零成本：
...
最可靠的安全网。详细 TDD 工作流参考[测试指南](/zh/blog/claude-code-testing-guide)：
...
好的 CLAUDE.md 配置能让 Claude Code 在重构时自动遵循团队规范。完整用法参考 [CLAUDE.md 指南](/zh/blog/claude-md-guide)。
...
配合[调试指南](/zh/blog/claude-code-debugging-guide)一起使用——调试解决"坏了怎么修"，重构解决"没坏但可以更好"。

## 推荐阅读

- [Claude Code 进阶指南](/zh/blog/claude-code-advanced-guide) — 基础重构流程和 Plan Mode 用法
- [Claude Code 调试工作流指南](/zh/blog/claude-code-debugging-guide) — 姊妹篇，系统性调试方法论
- [Claude Code 测试工作流指南](/zh/blog/claude-code-testing-guide) — TDD、Coverage、测试驱动重构的基础
- [Claude Code Git 工作流指南](/zh/blog/claude-code-git-guide) — Worktree 隔离、分支策略、Commit 规范
- [Claude Code Prompt 技巧指南](/zh/blog/claude-code-prompt-guide) — 写出高质量重构提示词的方法
- [Claude Code 多 Agent 指南](/zh/blog/claude-code-multi-agent-guide) — 并行重构的多 Agent 策略
- [CLAUDE.md 指南](/zh/blog/claude-md-guide) — 完整的 CLAUDE.md 配置方法
- [Claude Code Slash Commands 指南](/zh/blog/claude-code-skills-guide) — /review 命令在重构中的应用
- [Claude Code 上下文管理指南](/zh/blog/claude-code-context-guide) — 大规模重构时的上下文控制
- [Claude Code Hooks 指南](/zh/blog/claude-code-hooks-guide) — 自动化重构验证的 Hook 配置
