...
调试是开发者日常中最耗时、最考验耐心的环节。你可能在 [测试指南](/zh/blog/claude-code-testing-guide) 的第六章里见过"调试失败测试"的内容，但那只是调试工作流的冰山一角。测试失败只是 bug 的一种表现形式——实际开发中，你面对的可能是一个莫名其妙的运行时报错、一个只在生产环境出现的诡异行为、一个拖慢页面加载的性能瓶颈，或者一个让你抓狂的异步竞态条件。

这篇文章要做的，就是系统地讲清楚：**怎么用 Claude Code 建立一套完整的调试工作流**——从看到报错的第一秒，到定位根因、修复问题、防止回归的全过程。

如果你刚读完 [重构指南](/zh/blog/claude-code-refactoring-guide)，会发现调试和重构是天然的搭档：调试帮你找到问题，重构帮你从根本上消除问题。两篇文章配合阅读效果更好。
...
当你知道"某个版本是好的，当前版本是坏的"，`git bisect` 是神器。[Git 指南](/zh/blog/claude-code-git-guide)里介绍过基础用法，这里讲怎么和 Claude Code 配合：
...
在 [CLAUDE.md 指南](/zh/blog/claude-md-guide) 里我们讲过如何配置项目规范。这一章专门讲调试相关的规范——把团队的调试经验沉淀下来，让 Claude Code 每次都按最佳实践来。
...
记住，最好的调试是不需要调试——好的代码规范、完善的测试、清晰的错误处理，能在 bug 出现之前就把它们挡在门外。这也是为什么这篇文章和 [测试指南](/zh/blog/claude-code-testing-guide)、[重构指南](/zh/blog/claude-code-refactoring-guide) 是三位一体的：测试预防 bug，调试定位 bug，重构消除 bug 的土壤。

## 推荐阅读

- [Claude Code 进阶指南](/zh/blog/claude-code-advanced-guide) — Claude Code 的进阶用法和高级技巧
- [Claude Code 测试工作流指南](/zh/blog/claude-code-testing-guide) — TDD、Mock、Coverage 和失败测试调试
- [Claude Code 重构工作流指南](/zh/blog/claude-code-refactoring-guide) — 代码重构的系统方法论，调试的天然搭档
- [Claude Code Git 工作流指南](/zh/blog/claude-code-git-guide) — git bisect 等调试相关的 Git 技巧
- [Claude Code Prompt 技巧指南](/zh/blog/claude-code-prompt-guide) — 写出更精准的调试 prompt
- [CLAUDE.md 编写指南](/zh/blog/claude-md-guide) — 把调试规范写入项目配置
- [Claude Code 上下文管理指南](/zh/blog/claude-code-context-guide) — 管理调试时的上下文窗口
- [Claude Code Hooks 指南](/zh/blog/claude-code-hooks-guide) — 自动化调试工作流的钩子机制
- [Claude Code MCP Server 指南](/zh/blog/claude-code-mcp-guide) — 扩展调试能力的 MCP 工具
- [Claude Code Settings 指南](/zh/blog/claude-code-settings-guide) — 调试相关的编辑器和工具配置
- [Claude Code 多 Agent 指南](/zh/blog/claude-code-multi-agent-guide) — 多 Agent 协作调试复杂问题
- [Claude Code Slash Commands 指南](/zh/blog/claude-code-skills-guide) — 自定义调试命令模板