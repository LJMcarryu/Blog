...
在 [CLAUDE.md 完全指南](/zh/blog/claude-md-guide)里，我提过一句话：**CLAUDE.md 告诉 Claude "做什么"，settings.json 告诉 Claude "怎么做"。**

之后的几篇文章分别深入了 settings.json 的局部：[Hooks 指南](/zh/blog/claude-code-hooks-guide)讲了 `hooks` 字段，[MCP 指南](/zh/blog/claude-code-mcp-guide)讲了 `mcpServers` 字段。但 settings.json 作为一个整体——它的作用域层级、合并机制、权限系统——一直没有专门讲过。
...
| `hooks` | object | 生命周期钩子 | [Hooks 指南](/zh/blog/claude-code-hooks-guide) |
| `mcpServers` | object | MCP 服务器配置 | [MCP 指南](/zh/blog/claude-code-mcp-guide) |
...
Hooks 遵循对象合并规则。同一个生命周期事件（如 `PostToolUse`）的 hooks 数组会拼接。详细的合并行为参考 [Hooks 指南](/zh/blog/claude-code-hooks-guide)。
...
- [CLAUDE.md 完全指南](/zh/blog/claude-md-guide) — settings.json 的搭档，管"做什么"
- [Claude Code Hooks 指南](/zh/blog/claude-code-hooks-guide) — settings.json 中 hooks 字段的深度解析
- [上下文管理指南](/zh/blog/claude-code-context-guide) — 管好上下文，让配置更高效
- [多 Agent 并行指南](/zh/blog/claude-code-multi-agent-guide) — 多终端协作的配置策略
- [自定义 Slash Commands 指南](/zh/blog/claude-code-skills-guide) — 把常用操作变成一键命令
- [MCP Server 深度指南](/zh/blog/claude-code-mcp-guide) — settings.json 中 mcpServers 字段的深度解析
- [高效 Prompt 指南](/zh/blog/claude-code-prompt-guide) — 配置好环境后，写好 Prompt 是下一步