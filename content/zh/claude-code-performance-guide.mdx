...
在之前的系列文章中，我们讲了[上下文管理](/zh/blog/claude-code-context-guide)的操作细节，讲了[怎么写好 Prompt](/zh/blog/claude-code-prompt-guide)，也在 [CI/CD 指南](/zh/blog/claude-code-cicd-guide)里提到了一些成本控制的基础策略。但一直缺一篇从**性能和成本**视角系统性地讲优化的文章。
...
你可以在 settings.json 中设置默认模型，避免每次手动切换（详见 [settings 指南](/zh/blog/claude-code-settings-guide)）：
...
> 关于上下文管理的具体操作命令和技巧，请参考[上下文管理指南](/zh/blog/claude-code-context-guide)。这里我们从成本角度讲"为什么"和"策略"。
...
好的 Prompt 不仅能提高输出质量，还能直接省钱。这不是重复[Prompt 指南](/zh/blog/claude-code-prompt-guide)的内容，而是从成本角度重新审视 Prompt 的写法。
...
[Prompt 指南](/zh/blog/claude-code-prompt-guide)里的技巧，从成本角度重新解读：
...
Claude Code 支持多 Agent 并行执行（详见[多 Agent 指南](/zh/blog/claude-code-multi-agent-guide)）。从成本角度看：
...
自定义 slash 命令（详见 [Skills 指南](/zh/blog/claude-code-skills-guide)）不仅提高效率，还能省钱：
...
> 完整的 CI/CD 配置请参考 [CI/CD 指南](/zh/blog/claude-code-cicd-guide)。这里聚焦成本控制策略。
...
[Hooks](/zh/blog/claude-code-hooks-guide) 在每次工具调用时执行，如果 Hook 脚本本身很慢，会显著影响整体体验。
...
MCP（Model Context Protocol）工具（详见 [MCP 指南](/zh/blog/claude-code-mcp-guide)）引入了外部服务调用，每次调用都有网络延迟。
...
- [上下文管理指南](/zh/blog/claude-code-context-guide) — 上下文管理的完整操作手册
- [Prompt 指南](/zh/blog/claude-code-prompt-guide) — 写出高质量 Prompt 的技巧
- [CI/CD 指南](/zh/blog/claude-code-cicd-guide) — CI/CD 环境下的完整配置
- [多 Agent 指南](/zh/blog/claude-code-multi-agent-guide) — 多 Agent 并行架构
- [Settings 指南](/zh/blog/claude-code-settings-guide) — 模型和工具的详细配置
- [Hooks 指南](/zh/blog/claude-code-hooks-guide) — Hook 系统的配置和优化
- [MCP 指南](/zh/blog/claude-code-mcp-guide) — MCP 工具的使用和配置
- [Skills 指南](/zh/blog/claude-code-skills-guide) — 自定义命令的创建和复用
