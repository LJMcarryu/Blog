...
CLAUDE.md 是 Claude Code 的**「员工手册」**。它告诉 Claude "做什么"——项目怎么构建、代码风格是什么、有哪些坑要避开。而 [settings.json](/zh/blog/claude-code-settings) 告诉 Claude "怎么做"——权限、模型、工具配置。两者搭配，才是完整的 Claude Code 配置体系。
...
**修正：** 配置好 Prettier/ESLint，在 CLAUDE.md 里只写"提交前运行 `npm run lint:fix`"。如果你配了 [Hooks](/zh/blog/claude-code-hooks)，连这句都不用写——Hook 会自动格式化。
...
- [Claude Code settings.json 完全指南](/zh/blog/claude-code-settings) — CLAUDE.md 的搭档，管"怎么做"
- [上下文管理指南](/zh/blog/claude-code-context) — 管好上下文，让 CLAUDE.md 更高效
- [Claude Code Hooks 指南](/zh/blog/claude-code-hooks) — 用 Hooks 自动化代码格式化等重复工作
- [高效 Prompt 指南](/zh/blog/claude-code-prompt) — 配置好 CLAUDE.md 后，写好 Prompt 是下一步
- [自定义 Slash Commands 指南](/zh/blog/claude-code-skills) — 把常用操作变成一键命令
- [MCP Server 深度指南](/zh/blog/claude-code-mcp) — 用 MCP 扩展 Claude Code 的能力
