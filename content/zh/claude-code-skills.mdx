...
在[进阶指南](/zh/blog/claude-code-overview)的第五节里，我用大约 40 行介绍了自定义 Slash Commands 的基本用法：创建 `.claude/commands/` 目录，写几个 `.md` 文件，用 `$ARGUMENTS` 传参。
...
Slash Commands 和 [Hooks](/zh/blog/claude-code-hooks) 是互补的：
...
- [Claude Code Hooks 指南](/zh/blog/claude-code-hooks) — 确定性自动化，与 Slash Commands 互补
- [上下文管理指南](/zh/blog/claude-code-context) — 管好上下文，让命令执行更精准
- [多 Agent 并行指南](/zh/blog/claude-code-multi-agent) — 用命令 + 多终端实现团队级效率
